...
const ProductDetail = () => {
  ...
  // ------ Ref lưu vị trí touch zoom trước đó ------
  const lastZoomTouchRef = useRef(null)
  ...
  // ------ Hàm cập nhật vị trí zoom image ------
  ...
  // ------ Hàm cập nhật vị trí zoom theo hướng kéo ------
  const moveZoomPosition = (deltaX, deltaY) => {
    ...
    setZoomPosition((prev) => {
      ...
      // ------ Khai báo const next x ------
      const nextX = prev.x + (deltaX / rect.width) * 100

      // ------ Khai báo const next y ------
      const nextY = prev.y + (deltaY / rect.height) * 100
  ...
  // ------ Hàm xử lý touch start zoom ------
  const handleTouchStartZoom = (e) => {
    if (!isZoomMode || e.touches.length === 0) return

    // ------ Khai báo const touch ------
    const touch = e.touches[0]

    lastZoomTouchRef.current = {
      x: touch.clientX,
      y: touch.clientY,
    }
  }

  // ------ Hàm xử lý touch move zoom ------
  const handleTouchMoveZoom = (e) => {
    ...
    if (lastZoomTouchRef.current) {
      moveZoomPosition(
        touch.clientX - lastZoomTouchRef.current.x,
        touch.clientY - lastZoomTouchRef.current.y,
      )
    }
  ...
  // ------ Hàm xử lý touch end zoom ------
  const handleTouchEndZoom = () => {
    lastZoomTouchRef.current = null
  }

  // ------ Hàm xử lý toggle zoom ------
      ...
      <div className="mx-auto grid max-w-7xl gap-8 px-4 lg:grid-cols-2">
        <section>
          <div
            ...
            onTouchStart={isZoomMode ? handleTouchStartZoom : undefined}
            onTouchMove={isZoomMode ? handleTouchMoveZoom : undefined}
            onTouchEnd={isZoomMode ? handleTouchEndZoom : undefined}
            onTouchCancel={isZoomMode ? handleTouchEndZoom : undefined}
          ...
          >
            ...
            {isZoomMode && (
              <div className="pointer-events-none absolute bottom-4 left-1/2 z-30 -translate-x-1/2 rounded-full bg-black/60 px-4 py-2 text-sm font-bold text-white hidden lg:block">
